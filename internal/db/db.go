...
func (r *repository) PipelineTemplateSave(ctx context.Context, t pipeline.PipelineTemplate) (int, error) {
	id := 0
	conditionsBytes, err := json.Marshal(t.Conditions)
	if err != nil {
		return id, fmt.Errorf("failed to marshall tempalate conditions")
	}

	queryBytes, err := json.Marshal(t.Query)
	if err != nil {
	...
	// r.logger.Traceln("SQL Query:", formatQuery(query))
	r.client.QueryRow(ctx, query,
		t.EventName, conditionsBytes, queryBytes,
		t.ExitPipelineName, t.NextPipelineId, t.ExecuteDelay,
		t.IsActive, t.ExecuteType).Scan(&id)
	if id == 0 {
...
// If no results are found or database errors occur, the error is propagated upward.
func (r *repository) PipelineTemplatesLoad(ctx context.Context) ([]pipeline.PipelineTemplate, error) {
	query := `
	SELECT 
		id,
		event_name,
	...
	tArr := make([]pipeline.PipelineTemplate, 0)

	for rows.Next() {
		t := pipeline.PipelineTemplate{}

		err = rows.Scan(&t.Id, &t.EventName, &t.Conditions,
			&t.Query, &t.ExitPipelineName, &t.NextPipelineId,
			&t.ExecuteDelay, &t.IsActive, &t.ExecuteType)
		if err != nil {
			return nil, err
		}
		tArr = append(tArr, t)
	...
	if len(tArr) == 0 {
		return tArr, fmt.Errorf("no templates found")
	}
...
func (r *repository) PipelineTemplateLoad(ctx context.Context, templateId int) (pipeline.PipelineTemplate, error) {
	...
	err := row.Scan(&tempalate.Id, &tempalate.EventName, &tempalate.Conditions,
		&tempalate.Query, &tempalate.ExitPipelineName, &tempalate.NextPipelineId,
		&tempalate.ExecuteDelay, &tempalate.IsActive, &tempalate.ExecuteType)
	if err != nil {
